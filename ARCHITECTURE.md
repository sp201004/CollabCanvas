...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  1. handlePointerDown()                                     │  │
│  │     - Generate strokeId (nanoid)                            │  │
│  │     - Create Stroke object with tool, color, width          │  │
│  │     - Add first point to local stroke                       │  │
│  │     - Render point on dynamicCanvas (immediate feedback)    │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                 │                                 │
│                                 ▼                                 │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  2. handlePointerMove() [Continuous while drawing]          │  │
│  │     - Collect points in pointBuffer[]                       │  │
│  │     - Render smoothed line on dynamicCanvas (interpolated)  │  │
│  │     - Emit batched points every 50ms (debounced)            │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                 │                                 │
│                                 ▼                                 │
│  ┌─────────────────────────────────────────────────────────────┐  │
...
│  │     - Transfer stroke from dynamicCanvas to staticCanvas    │  │
│  │     - Clear dynamicCanvas                                   │  │
...
│  │  - stroke:start  → { stroke, roomId }                       │  │
│  │  - stroke:point  → { strokeId, point, roomId } [batched]    │  │
│  │  - stroke:end    → { strokeId, roomId }                     │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Event Handlers                                             │  │
│  │  1. stroke:start → Store stroke in room.strokes[]           │  │
│  │  2. stroke:point → Append point to stroke.points[]          │  │
│  │  3. stroke:end   → Add to operationHistory                  │  │
│  │                  → Save to .canvas-data/<roomId>.json       │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                 │                                 │
│                                 ▼                                 │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Broadcast to Room (io.to(roomId).emit)                     │  │
│  │  - stroke:start  → All clients except sender                │  │
│  │  - stroke:point  → All clients except sender                │  │
│  │  - stroke:end    → All clients except sender                │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                        WebSocket Broadcast
                                 ...
                                 ▼
┌───────────────────────────────────────────────────────────────────┐
│                     Other Clients (Remote Users)                  │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  useSocket Hook - Event Listeners                           │  │
│  │  1. stroke:start → Create new stroke in remoteStrokes[]     │  │
│  │  2. stroke:point → Append point to remote stroke            │  │
│  │                  → Render on dynamicCanvas                  │  │
│  │  3. stroke:end   → Transfer to staticCanvas                 │  │
...
Timing: 
- Point batching: ~20ms between points (50 FPS capture)
- Network latency: 10-50ms depending on connection
- Total lag: 30-70ms for remote users (imperceptible)
```
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  1. handlePointerDown()                                     │  │
│  │     - Record startPoint { x, y }                            │  │
│  │     - Set isDrawingShape = true                             │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                 │                                 │
│                                 ▼                                 │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  2. handlePointerMove() [While dragging]                    │  │
│  │     - Calculate currentPoint { x, y }                       │  │
│  │     - Clear dynamicCanvas                                   │  │
│  │     - Redraw staticCanvas onto dynamicCanvas (background)   │  │
│  │     - Draw preview shape (startPoint → currentPoint)        │  │
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  3. handlePointerUp()                                       │  │
│  │     - Calculate final dimensions (width, height, radius)    │  │
│  │     - Generate shapeId (nanoid)                             │  │
│  │     - Create Shape object with type, position, dimensions   │  │
│  │     - Emit shape:add event                                  │  │
│  │     - Clear isDrawingShape flag                             │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  shape:add Handler                                          │  │
│  │  1. Store shape in room.shapes[]                            │  │
│  │  2. Add operation to operationHistory                       │  │
│  │  3. Clear undoneOperations stack                            │  │
│  │  4. Save to .canvas-data/<roomId>.json                      │  │
│  │  5. Broadcast shape:add to ALL clients (including sender)   │  │
│  │  6. Broadcast history:state to update undo/redo buttons     │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                        WebSocket Broadcast
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  shape:add Listener                                         │  │
│  │  1. Add shape to local shapes[] array                       │  │
│  │  2. Redraw entire canvas:                                   │  │
│  │     - Clear staticCanvas                                    │  │
│  │     - Redraw all strokes (from bottom to top)               │  │
│  │     - Redraw all shapes (in order)                          │  │
│  │  3. Shape appears instantly for all users                   │  │
│  └─────────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────────┘

Note: Shapes are atomic operations (sent once on mouse up), unlike strokes 
      which are streamed incrementally. This eliminates mid-draw sync issues.
```
...
┌───────────────────────────────────────────────────────────────────┐
│                           User Action                             │
│               (Click Undo/Redo button in ToolPanel)               │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
┌───────────────────────────────────────────────────────────────────┐
...
│  │  - Check if canUndo/canRedo is true                         │  │
│  │  - Call onUndo() or onRedo() callback                       │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  │  - Emit operation:undo or operation:redo                    │  │
│  │  - No local state change (wait for server broadcast)        │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Emit Event (Client → Server)                               │  │
│  │  - operation:undo → { roomId }                              │  │
│  │  - operation:redo → { roomId }                              │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                        WebSocket Connection
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Undo Handler                                               │  │
│  │  1. Check if operationHistory.length > 0                    │  │
│  │  2. Pop last operation from operationHistory                │  │
│  │  3. Push operation to undoneOperations                      │  │
│  │  4. Determine operation type:                               │  │
│  │     - If "draw" → Remove stroke from room.strokes[]         │  │
│  │     - If "shape" → Remove shape from room.shapes[]          │  │
│  │  5. Save updated state to .canvas-data/<roomId>.json        │  │
│  │  6. Broadcast operation:undo to ALL clients                 │  │
│  │  7. Broadcast history:state with updated counts             │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                 │                                 │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Redo Handler (Similar flow)                                │  │
│  │  1. Check if undoneOperations.length > 0                    │  │
│  │  2. Pop operation from undoneOperations                     │  │
│  │  3. Push operation to operationHistory                      │  │
│  │  4. Restore stroke/shape to room state                      │  │
│  │  5. Save + Broadcast operation:redo                         │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                        WebSocket Broadcast
...
│                   All Clients (Including Sender)                  │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  operation:undo / operation:redo Listener                   │  │
│  │  1. Receive undone operation details from server            │  │
│  │  2. Update local state:                                     │  │
│  │     - Remove/restore stroke from strokes[]                  │  │
│  │     - Remove/restore shape from shapes[]                    │  │
│  │  3. Redraw entire canvas from scratch:                      │  │
│  │     - Clear staticCanvas and dynamicCanvas                  │  │
│  │     - Render all remaining strokes (in order)               │  │
│  │     - Render all remaining shapes                           │  │
│  │  4. Listen for history:state event                          │  │
│  │  5. Update canUndo/canRedo button states                    │  │
│  └─────────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────────┘

Key Design Decisions:
- Server is source of truth for history (prevents conflicts)
- Any user can undo ANY operation (global undo, not per-user)
- All clients redraw from shared state (ensures consistency)
- Redo stack clears when new operation is performed
```
...
┌───────────────────────────────────────────────────────────────────┐
│                          User Action                              │
│             (Enter room code + username, click Join)              │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
┌───────────────────────────────────────────────────────────────────┐
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  handleJoinRoom()                                           │  │
│  │  - Validate roomId (6 chars alphanumeric)                   │  │
│  │  - Validate username (1-20 chars)                           │  │
│  │  - Navigate to /canvas/:roomId                              │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  │  2. Retrieve username from localStorage                     │  │
│  │  3. Connect to Socket.io server                             │  │
│  │  4. Emit room:join event                                    │  │
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  room:join Handler                                          │  │
│  │  1. Create room if doesn't exist (rooms.set)                │  │
│  │  2. Load persisted state from .canvas-data/<roomId>.json:   │  │
│  │     - If file exists → Load strokes + operationHistory      │  │
│  │     - If no file → Initialize empty arrays                  │  │
│  │  3. Generate userId + assign random color                   │  │
│  │  4. Add user to room.users Map                              │  │
│  │  5. socket.join(roomId) - Join Socket.io room               │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                 │                                 │
│                    ┌────────────┼────────────┐                    │
│                    │            │            │                    │
│                    ▼            ▼            ▼                    │
│  ┌──────────────────────┐ ┌──────────┐ ┌──────────────────────┐   │
│  │ Emit to New Client   │ │ Broadcast│ │ Broadcast to Room    │   │
│  │ (socket.emit)        │ │ to Room  │ │ (io.to(roomId).emit) │   │
│  │                      │ │          │ │                      │   │
│  │ • room:joined        │ │ • user:  │ │ • user:joined        │   │
│  │   - userId           │ │   list   │ │   - New user info    │   │
│  │   - username         │ │   - All  │ │                      │   │
│  │   - color            │ │     users│ │ • history:state      │   │
│  │                      │ │          │ │   - operationCount   │   │
│  │ • canvas:state       │ │          │ │   - undoneCount      │   │
│  │   - strokes[]        │ │          │ │                      │   │
│  │   - shapes[]         │ │          │ │                      │   │
│  │                      │ │          │ │                      │   │
│  │ • user:list          │ │          │ │                      │   │
...
└────────────────────────────────┬──────────────────────────────────┘
                        ...
                        WebSocket Events
                                 ...
                                 ▼
┌───────────────────────────────────────────────────────────────────┐
│                      New Client (Joining User)                    │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Event Listeners                                            │  │
│  │  1. room:joined → Store userId, username, color             │  │
│  │  2. canvas:state → Store strokes[] and shapes[]             │  │
│  │  3. user:list → Display all online users                    │  │
│  │  4. history:state → Enable/disable undo/redo buttons        │  │
│  │  5. Set isLoading = false                                   │  │
│  │  6. Render canvas with all existing strokes/shapes          │  │
│  └─────────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────────┘
                                 │
┌───────────────────────────────────────────────────────────────────┐
│                  Existing Clients (Already in Room)               │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Event Listeners                                            │  │
│  │  1. user:joined → Add new user to users[] list              │  │
│  │  2. Show toast notification: "Username joined the room"     │  │
│  │  3. Render new user's cursor when they move                 │  │
│  └─────────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────────┘

Synchronization Guarantees:
- New users receive COMPLETE canvas state before drawing
- Loading state prevents premature rendering
- Server is single source of truth (no client-side assumptions)
- Race condition handling: room:joined must arrive before canvas:state
```
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  handlePointerMove()                                        │  │
│  │  1. Get cursor position (x, y) relative to canvas           │  │
│  │  2. Check if currently drawing (isDrawing flag)             │  │
│  │  3. Call onCursorMove callback with position + isDrawing    │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  handleCursorMove()                                         │  │
│  │  - Throttle to 100ms (avoid network spam)                   │  │
│  │  - Emit cursor:move event with position + isDrawing         │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  Emit Event (Client → Server)                               │  │
│  │  - cursor:move → { roomId, position: {x, y}, isDrawing }    │  │
│  │                                                             │  │
│  │  Throttling: Only 10 updates per second per user            │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
...
│  │  2. Get sender's userId from socket                         │  │
│  │  3. Create cursor update payload:                           │  │
│  │     {                                                       │  │
│  │       userId,                                               │  │
│  │       username,                                             │  │
│  │       position: { x, y },                                   │  │
...
│  │  1. Receive cursor data for remote user                     │  │
│  │  2. Update remoteCursors Map:                               │  │
│  │     remoteCursors.set(userId, {position, color, username})  │  │
│  │  3. RequestAnimationFrame to render cursor                  │  │
│  └─────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬──────────────────────────────────┘
                                 │
                                 ▼
...
│  │  - Map through remoteCursors                                │  │
│  │  - For each cursor:                                         │  │
│  │    • Draw SVG cursor icon at (x, y)                         │  │
│  │    • Apply user's assigned color                            │  │
│  │    • Show username label below cursor                       │  │
│  │    • Add smooth CSS transition (0.1s ease-out)              │  │
│  │    • If isDrawing → Add pulsing animation                   │  │
│  └─────────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────────┘

Performance Optimizations:
- Throttling: 10 updates/sec instead of 60 FPS (53% reduction in traffic)
- No echo: Cursor moves not sent back to sender
- RAF rendering: Smooth 60 FPS cursor animation despite 10Hz updates
- Interpolation: CSS transitions smooth out discrete position updates
```
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  handleWheel() - Zoom                                       │   │
│  │  1. Calculate zoom delta (wheel deltaY)                     │   │
│  │  2. Determine zoom direction (in/out)                       │   │
│  │  3. Calculate new scale:                                    │   │
│  │     newScale = currentScale * (1 + delta * 0.001)           │   │
│  │  4. Clamp scale between 0.1x and 5x                         │   │
│  │  5. Calculate mouse position in canvas space                │   │
│  │  6. Adjust pan offset to zoom toward mouse cursor           │   │
│  │  7. Update transform state                                  │   │
│  │  8. Trigger canvas redraw (requestAnimationFrame)           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                 │                                  │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  handlePointerMove() - Pan (with Space key)                 │   │
│  │  1. Check if Space key is pressed (isPanning = true)        │   │
│  │  2. Calculate drag delta (currentPos - lastPos)             │   │
│  │  3. Update pan offset:                                      │   │
│  │     panOffset.x += delta.x                                  │   │
│  │     panOffset.y += delta.y                                  │   │
│  │  4. Update cursor style to "grabbing"                       │   │
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  redrawCanvas()                                             │   │
│  │  1. Get 2D context from staticCanvas                        │   │
│  │  2. Save current context state (ctx.save())                 │   │
│  │  3. Clear entire canvas                                     │   │
│  │  4. Apply transform matrix:                                 │   │
│  │     ctx.translate(panOffset.x, panOffset.y)                 │   │
│  │     ctx.scale(scale, scale)                                 │   │
│  │  5. Redraw all strokes with transform applied               │   │
│  │  6. Redraw all shapes with transform applied                │   │
│  │  7. Restore context state (ctx.restore())                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────────────┘

Note: Pan & Zoom are CLIENT-SIDE ONLY
- Not synchronized across users (each user has independent view)
- No WebSocket events emitted
- Transform only affects local rendering, not stored stroke data
- Strokes stored in absolute coordinates (0,0 origin)
- Cursor positions sent in absolute coordinates (transform applied during render)
```
...
│                         Client (Browser)                        │
│  ┌────────────────────┐                                         │
│  │  localStorage      │  Auto-save on:                          │
│  │  (Backup Layer)    │  - stroke:end                           │
│  │                    │  - undo/redo                            │
│  │  Key Format:       │  - canvas:clear                         │
│  │  collabcanvas:     │                                         │
│  │  <roomId>:state    │  Fallback only, not used if server      │
│  └────────────────────┘  has state                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                        WebSocket Sync
...
│                         Server (Node.js)                        │
│  ┌────────────────────┐                                         │
│  │  .canvas-data/     │  Auto-save on:                          │
│  │  (Source of Truth) │  - stroke:end                           │
│  │                    │  - undo/redo                            │
│  │  <roomId>.json     │  - canvas:clear                         │
│  │                    │                                         │
│  │  Persists:         │  Restored on:                           │
│  │  - strokes[]       │  - room:join (new user)                 │
│  │  - operations[]    │  - server restart                       │
│  │  - timestamp       │  - empty room rejoin                    │
│  └────────────────────┘                                         │
└─────────────────────────────────────────────────────────────────┘
```
